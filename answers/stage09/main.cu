...
// Eliminates shared memory bank conflicts by switching to the reversed sequential addressing loop.
...
int runReductionDemo() {
    ...
    for (int run = 0; run < kGpuRuns; ++run) {
        ...
        CHECK_CUDA(reduceSharedMemorySequentialAddressing(hostInput.data(), hostInput.size(), &gpuSum));
    ...
    if (relativeError <= kRelativeTolerance) {
        std::cout << "Stage 9 reduction matches reference ✅" << std::endl;
    } else {
        std::cout << "Stage 9 reduction mismatch ❌" << std::endl;
    }
