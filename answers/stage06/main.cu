...
// This demonstrates tiled matrix multiplication using shared memory for better performance.
...
void fillMatrix(Matrix matrix, float scale) {
    for (int row = 0; row < matrix.height; ++row) {
        for (int col = 0; col < matrix.width; ++col) {
            const int idx = row * matrix.stride + col;
    ...
    for (int row = 0; row < a.height; ++row) {
        for (int col = 0; col < b.width; ++col) {
            ...
            for (int k = 0; k < a.width; ++k) {
                sum += a.elements[row * a.stride + k] * b.elements[k * b.stride + col];
            }
            out.elements[row * out.stride + col] = sum;
        }
    }
}
...
bool compareMatrices(const Matrix& lhs, const Matrix& rhs, float tolerance) {
    ...
    for (int row = 0; row < lhs.height; ++row) {
        for (int col = 0; col < lhs.width; ++col) {
            const float lval = lhs.elements[row * lhs.stride + col];
            const float rval = rhs.elements[row * rhs.stride + col];
            const float diff = std::fabs(lval - rval);
            const float relativeError = diff / std::fmax(std::fabs(rval), 1e-6f);
    ...
    if (errors > 0) {
        const int total = lhs.width * lhs.height;
        std::cerr << "Total errors: " << errors << " / " << total
                  << " (" << (100.0 * errors / total) << "%)\n";
        return false;
    }
    return true;
...
int runMatrixMulDemo() {
    ...
    Matrix hostA{K, M, K, hostAData.data()};  // A: M x K (width=K, height=M, stride=K)
    Matrix hostB{N, K, N, hostBData.data()};  // B: K x N (width=N, height=K, stride=N)
    Matrix hostC{N, M, N, hostCData.data()};  // C: M x N (width=N, height=M, stride=N)
#ifndef SKIP_CPU
    Matrix reference{N, M, N, hostReferenceData.data()};
#endif
    ...
    for (int row = 0; row < hostC.height; ++row) {
        for (int col = 0; col < hostC.width; ++col) {
            std::cout << hostC.elements[row * hostC.stride + col] << std::endl;
        }
    }
#else
    // Verification
    if (compareMatrices(hostC, reference, kTolerance)) {
        std::cout << "Stage 6 matrix multiplication matches reference ✅" << std::endl;
    } else {
        std::cout << "Stage 6 matrix multiplication mismatch ❌" << std::endl;
    }

    // Performance comparison
    std::cout << "\nPerformance Comparison:" << std::endl;
    std::cout << "  CPU Time: " << cpuTime << " ms" << std::endl;
    std::cout << "  GPU Time (with shared memory): " << gpuTime << " ms" << std::endl;
    std::cout << "  Speedup vs CPU: " << (cpuTime / gpuTime) << "x" << std::endl;
#endif
