...
// Keeps the first-add-during-load pattern and removes the final warp's control-flow overhead.
...
static_assert((kElementCount % (kThreadsPerBlock * kElementsPerThread)) == 0,
              "Stage 11 expects the element count to be a multiple of blockDim.x * 2");
...
int runReductionDemo() {
    ...
    for (int run = 0; run < kGpuRuns; ++run) {
        ...
        CHECK_CUDA(reduceSharedMemoryFirstAddWarpUnroll(
    ...
    if (relativeError <= kRelativeTolerance) {
        std::cout << "Stage 11 reduction matches reference ✅" << std::endl;
    } else {
        std::cout << "Stage 11 reduction mismatch ❌" << std::endl;
    }
