...
__global__ void reduceSharedMemoryFirstAddKernel(const float* g_idata,
    ...
    // TODO: Compute the per-thread base index (blockIdx.x * blockDim.x * 2 + tid).
    // TODO: Load g_idata[baseIdx] and g_idata[baseIdx + blockDim.x], add them immediately,
    // and store the sum in shared memory.
    __syncthreads();
