...
// Threads load two elements each, accumulate immediately, and then run the sequential addressing loop.
...
static_assert((kElementCount % (kThreadsPerBlock * kElementsPerThread)) == 0,
              "Stage 10 expects the element count to be a multiple of blockDim.x * 2");
...
int runReductionDemo() {
    ...
    for (int run = 0; run < kGpuRuns; ++run) {
        ...
        CHECK_CUDA(reduceSharedMemoryFirstAddDuringLoad(
            hostInput.data(), hostInput.size(), &gpuSum));
    ...
    if (relativeError <= kRelativeTolerance) {
        std::cout << "Stage 10 reduction matches reference ✅" << std::endl;
    } else {
        std::cout << "Stage 10 reduction mismatch ❌" << std::endl;
    }
